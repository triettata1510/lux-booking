...
export default function BookPage() {
  ...
  // load services
  useEffect(() => {
    fetch("/api/services").then(r=>r.json()).then((d:Service[]) => {
      setServices(d || []);
  ...
  useEffect(() => {
    fetch("/api/technicians").then(r=>r.json()).then((d:Technician[]) => {
      setTechs(d || []);
    });
  }, []);
    ...
    try {
      const res = await fetch("/api/bookings", {
        ...
        body: JSON.stringify({
          ...
          start_at: slot,
          technician_id: technicianId || null, // optional
  ...
  return (
    <main className="mx-auto max-w-2xl p-6 space-y-6">
      ...
      <div>
        ...
        <select value={technicianId} onChange={e=>setTechnicianId(e.target.value)} className="w-full border rounded p-2">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input value={phone} onChange={e=>setPhone(e.target.value)} className="w-full border rounded p-2" placeholder="9 or 10 digits" />
        </div>
      </div>
